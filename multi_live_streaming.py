...
def webstreaming(args,stream_active,shape):
    app = Flask(__name__)
    socketio=socketio=SocketIO(app,async_mode='threading')

    upload_shm = SharedMemory(name = 'stream', create=False)
    upload_frame = np.ndarray(shape,dtype=np.uint8,buffer=upload_shm.buf)
    ...
    while check:
        if cap.zed.grab() == sl.ERROR_CODE.SUCCESS:
            ...
            if (x==0):  #to run once
                ...
                upload_shm = SharedMemory(name = 'stream',create=True, size=img.nbytes)
                upload_frames = np.ndarray(shape, dtype=img.dtype, buffer=upload_shm.buf)
                ...
                upload_process = multiprocessing.Process(target = webstreaming, args=(args,stream_active,shape))
